...
class uart_scoreboard extends uvm_scoreboard;
    ...
    function void write(uart_seq_item item);
        if (item.tx_data !== item.rx_data) begin
            ...
            `uvm_error(get_type_name(), $sformatf(
                       "Mistmatch !! tx_data= 0x%02h, rx_data = 0x%02h",
                       item.tx_data,
                       item.rx_data
                       ));
        end else begin
    ...
    function void report_phase(uvm_phase phase);
        ...
        if (fail_cnt > 0) begin
            `uvm_error(get_type_name(), $sformatf(
                       "TEST FAILED: %0d mismatches detected!", fail_cnt));
        end else begin
            `uvm_info(get_type_name(), $sformatf(
    ...
    task run_phase(uvm_phase phase);
        ...
        forever begin
            ...
            if (u_if.mon_cb.tx_start) begin
                ...
                `uvm_info(get_type_name(), $sformatf(
                          "[TX] tx_data= 0x%02h", item.tx_data), UVM_HIGH);
            end
...
class uart_driver extends uvm_driver #(uart_seq_item);
    ...
    //         u_if.drv_cb.tx_start <= 1'b0;
    //         `uvm_info(get_type_name(), $sformatf(
    //                   "start transmit : tx_data = 0x%02h", item.tx_data),
    //                   UVM_HIGH);
    //         @(u_if.drv_cb);
    //         while (!u_if.drv_cb.tx_busy) @(u_if.drv_cb);  // until busy increase
    //         while (u_if.drv_cb.tx_busy) @(u_if.drv_cb);  // until busy decrease
    //         `uvm_info(get_type_name(), $sformatf(
    //                   "complete transmit : tx_data = 0x%02h", item.tx_data),
    //                   UVM_HIGH);
    ...
    //             get_type_name(),
    //             $sformatf(
    //                 "raw rx_valid=%0b, cb rx_valid=%0b, raw tx_busy=%0b, cb tx_busy=%0b",
    //                 u_if.rx_valid,
    //                 u_if.drv_cb.rx_valid,
    //                 u_if.tx_busy,
    //                 u_if.drv_cb.tx_busy
    //             ),
    //             UVM_MEDIUM)  // add example
    //         @(u_if.drv_cb);  // rx_data receive check 
    //         item.rx_data = u_if.drv_cb.rx_data;
    //         `uvm_info(get_type_name(), $sformatf(
    //                   "complete to receive in monitor : rx_data = 0x%02h",
    //                   item.rx_data
    //                   ), UVM_HIGH);
